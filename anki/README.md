...
- `0014-vedecky-vs-produkcni-sw.txt` — C5 (23 kartiček): vědecký vs. produkční SW (dvě řemesla, ne dvě úrovně kvality), „kvalitní na co?", past oběma směry, nebezpečná zóna (tichý přechod prototyp→páteř), 4 otázky prototyp/produkt, Brooks 9× (1975), jak AI posouvá kompromis (podlaha/objem/úsudek), návaznost na A3 (akcidentální/esenciální), technický dluh, námitky, propojení na C1/C2/A2/A5.
- `0019-verifikace-code-review.txt` — D2 (23 kartiček): verifikace (dělá to správnou věc správně, ne že tak vypadá), review = povinná vrstva, proč AI review ≠ lidský (záměr/objem/vzhled), šest otázek na diff, techniky (malé diffy, spusť na známé odpovědi, adversariální čtení), zelená ≠ důkaz (model posvětí vlastní chybu), ownership („napsala to AI" není obhajoba), Fagan 1976, námitky, propojení na C4/A1/A5/C3.
- `0020-bezpecnost-citliva-data.txt` — D3 (23 kartiček): vložení do cloudu = přenos třetí straně (log/trénink), první otázka „smí to opustit perimetr?", tři koše (volné/podmíněné/nikdy do cloudu), klasifikace i úryvků, HPC omezení (data residency/air-gap/smluvní režim) → self-hosted model, prompt injection (text z internetu není příkaz), hooks (B5) do nástroje, GDPR/OWASP, námitky, propojení na B5/B1/C1/D2.
- `0018-context-engineering.txt` — D1 (22 kartiček): context engineering (co model vidí, ne chytrá slova), model nevezme v úvahu, co nemá v kontextu, okno = vzácný zdroj (málo→domýšlí, moc→context rot), vrstvy (CLAUDE.md trvalé vs. zadání jednorázové), techniky (konkrétnost, příklad, odkaz na soubory, doménový jazyk), páka se přesouvá z psaní na kontext, prevence trust gapu (C4) vs. verifikace (D2), RAG/harness, námitky, propojení na B1/B3/A4/C4.
- `0021-licence-provenance-autorstvi.txt` — D4 (22 kartiček): licence/provenance/autorství (společný jmenovatel = dohledatelnost a odpovědnost), AI není autor (nese odpovědnost člověk), přiznání ≠ přenos odpovědnosti („model to napsal" = „spletl se Excel"), provenance = záznam původu (pilíř FAIR, drží reprodukovatelnost i odpovědnost), licence vstupní (contaminace GPL u velkých bloků) vs. výstupní (člověk zůstává autorem), politiky časopisů 2023/US Copyright Office, hygiena, námitky, propojení na C1/C2/D2/D3.
- `0022-zavadeni-v-tymu.txt` — D5 (22 kartiček): zavádění = ze znalosti jednotlivce schopnost skupiny (sociální, ne technický problém), zákaz i příkaz = stejná chyba (nahrazují úsudek pravidlem), rámec s hranicemi, tři páky (politika/vzdělávání/kultura), co do politiky, uč verifikaci ne tlačítka, junioři a deskilling (C3), kultura otevřenosti drží trust gap (C4), shadow IT, Rogers/Westrum, metrika „drží kvalita a učí se lidé", námitky, propojení na D2/D3/D4/C3/C4/B.
